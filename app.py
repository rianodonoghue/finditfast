...
def delete_menu_image():
    ...
    flash("Menu image deleted.", "ok")
    return redirect(url_for("b_dashboard"))

@app.post("/b/post/<int:post_id>/delete")
@require(kind="business")
def business_delete_post(post_id):
    _, business = current_user()
...
@app.post("/b/special/<int:special_id>/delete")
@require(kind="business")
def business_delete_special(special_id):
    _, business = current_user()

    s = db.session.get(DailySpecial, special_id)
    if not s:
        abort(404)
    ...
    db.session.delete(s)
    db.session.commit()
    flash("Daily special deleted.", "ok")
    return redirect(url_for("b_dashboard"))
